var SOAP = (function () {
    try {
        ...
        // Modificata pentru UNTRR de Cosmin Vestemean, 0744236760
        //
        ...
        return {
            createHeader: function () {
                var usr = 'UNTRR50',
                usrtoken = 'UsernameToken-3E27C19BC03290ABE6160865517399633',
                pwd = '50-UNTRR50t',
                d1 = new Date().toISOString(),
                w = wsseHeader(usr, pwd),
                digest = w[0],
                nonce = w[1],
                createdUTC = w[2],
                wsse1 = 'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd',
                wsu1 = 'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd',
                ...
                'xmlns:wsse="' + wsse1 + '" ' +
                'xmlns:wsu="' + wsu1 + '">' +
                '<wsse:UsernameToken wsu:Id="' + usrtoken + '">' +
                '<wsse:Username>' + usr + '</wsse:Username>' +
                '<wsse:Password Type="' + passType + '">' + digest + '</wsse:Password>' +
                '<wsse:Nonce EncodingType="' + encType + '">' + nonce + '</wsse:Nonce>' +
                //'<wsu:Created>' + d1 + '</wsu:Created>' +
        ...
        //Friendly error
        X.WARNING(e.message.replace(/"/g, ""), null);
    }
}
    ());